...
-- DELIMITER // 
-- CREATE TRIGGER duration_Insert_Update BEFORE INSERT ON music_track 
-- FOR EACH ROW BEGIN
...
#TASK 2 Триггеры
CREATE TABLE IF NOT EXISTS music.log (
...
DELIMITER |
CREATE TRIGGER musican_before_insert 
	BEFORE INSERT 
    ON musican 
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Попытка записи музыканта:  ", NEW.`name`));
        END;
|

DELIMITER |
CREATE TRIGGER musican_after_insert 
	AFTER INSERT 
    ON musican 
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Музыкант записан успешно: ", NEW.`name`));
        END;
|

DELIMITER |
CREATE TRIGGER musican_before_update
	BEFORE UPDATE 
    ON musican
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Попытка обновления музыканта: ", OLD.`name`));
        END;
|

DELIMITER |
CREATE TRIGGER musican_after_update
	AFTER UPDATE
    ON musican
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Музыкант обновлен успешно: ", OLD.`name`));
        END;
|

DELIMITER |
CREATE TRIGGER musican_before_delete
	BEFORE DELETE
    ON musican 
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Попытка удаления музыканта: ", OLD.`name`));
        END;
|

DELIMITER |
CREATE TRIGGER musican_after_delete
	AFTER DELETE
    ON musican
    FOR EACH ROW BEGIN 
		INSERT INTO log VALUES(null, concat("Музыкант удален успешно: ", OLD.`name`));
        END;
|

DELIMITER |
#INSERT INTO user (id, email, password) values (10, 'annaa@gmail.com', '12345'); 
INSERT INTO lable VALUES("SuperLable");
INSERT INTO musican VALUES(null, "mus1", "SuperLable");
    |
    
-- DELIMITER |
-- INSERT INTO musican (userid, name, phone)values (10, 'Annaanna', '123456');
-- |

DELIMITER |
UPDATE musican SET name='MusMus55' WHERE lable_name = "SuperLable";
|

DELIMITER |
DELETE FROM musican WHERE lable_name = "SuperLable";
| 
...
# Хранимые процедуры
/*
Возвращает максимум из двух натуральных чисел.
*/
-- DELIMITER |
-- DROP FUNCTION IF EXISTS getMax;
-- |
...
-- |
-- CREATE PROCEDURE average(in a int, in b int, out res real)
-- BEGIN
-- 	SET res = (a+b) / 2;
-- END;
-- |
-- CALL average(1, 2, @res);
-- |
-- SELECT(@res);
-- |
